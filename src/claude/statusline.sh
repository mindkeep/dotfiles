...
BRC=110  # 256-color index for BR; the model's blue, distinct from any segment
...
for s in "${segs[@]}"; do
  ...
  # Color BR only after the width math above, which must see plain text.
  bs=$'\033[38;5;'$BRC'm'$BR$'\033[38;5;'$col'm'
  out+=$(printf '\033[38;5;240m%s\033[0m\033[38;5;%sm%s\033[0m' "$sep" "$col" "${s//"$BR"/$bs}")
  len=$((len + ${#sep} + w))
  first=0; i=$((i + 1))
done
